...
app.get("/items", function (req, res) {
  if (req.session.loggedin) {
    var sql =
      'SELECT * FROM items FULL JOIN users on customer_id= user_id WHERE customer_id <> ? AND i_status <> "DELIVERED" AND i_status <> "ONTHEWAY"';
    connection.query(sql, [req.session.userid], function (
      err,
      results,
      fields
    ) {
...
app.get("/myitems", function (req, res) {
  if (req.session.loggedin) {
    var sql =
      "SELECT * FROM items FULL JOIN users on customer_id = user_id WHERE customer_id = ?";
    connection.query(sql, [req.session.userid], function (
      err,
      results,
      fields
...
app.post("/items", function (req, res) {
  ...
  var sql =
    "INSERT INTO items(i_name, i_desc, i_to, i_from, i_type, customer_id) VALUES('" +
    name +
    "', '" +
    desc +
    "', '" +
...
app.get("/items/:id", function (req, res) {
  if (req.session.loggedin) {
    var sql =
      "SELECT * FROM items FULL JOIN users on customer_id = user_id WHERE item_id = ?";
...
app.post("/items/:id", function (req, res) {
  if (req.session.loggedin) {
    ...
    var sql =
      "UPDATE items SET i_name = ?, i_desc = ?, i_to = ?, i_from = ?, i_type = ? WHERE item_id = ?";
    connection.query(
      sql,
      [name, desc, to, from, type, req.params.id],
...
app.get("/myorders", function (req, res) {
  if (req.session.loggedin) {
    var sql =
      "SELECT * FROM orders AS o, items AS i, users AS u WHERE o.item_id=i.item_id AND deliverer_id = ? AND i.customer_id=u.user_id";
    connection.query(sql, [req.session.userid], function (
      err,
      results,
      fields
...
app.get("/orders", function (req, res) {
  if (req.session.loggedin) {
    var sql =
      "SELECT * FROM orders AS o, items AS i, users AS u WHERE o.item_id=i.item_id AND o.deliverer_id=u.user_id AND o.item_id IN(SELECT item_id FROM items WHERE customer_id=?)";
    connection.query(sql, [req.session.userid], function (
      err,
      results,
      fields
...
app.post("/orders", function (req, res) {
  ...
  var sql1 =
    "INSERT INTO orders(item_id, deliverer_id, estimated_date, message) VALUES('" +
    itemid +
    "','" +
    delivererid +
    "','" +
...
app.post("/login", function (req, res) {
  ...
  if (email && password) {
    connection.query(
      "SELECT * FROM users WHERE u_email = ? AND password = ?",
      [email, password],
...
app.post("/signup", function (req, res) {
  ...
  var sql =
    "INSERT INTO users(u_fname, u_lname, u_email, u_mobile, password) VALUES('" +
    fname +
    "', '" +
    lname +
    "', '" +
